...
class SimpleDungeonGenerator {
    ...
    field $max_items_per_room : param = $max_monsters_per_room;
    field $min_size : param(min_room_size);
    field $max_size : param(max_room_size);
    field $player : param;

    my sub place_entities ( $room, $map, $monsters, $items ) {

        # 1 or more monsters
        for my $i ( 1 .. int rand( $monsters + 1 ) ) {
            my ( $x, $y ) = $room->random_point()->@*;
            if ( !$map->has_entity_at( $x, $y ) ) {
                my $e =
                  rand() < 0.8 ? Entities::goblin() : Entities::hobgoblin();
                $map->spawn( $e, $x, $y );
            }
        }

        # 0 or more items
        for my $i ( 0 .. int rand($items) ) {
            my ( $x, $y ) = $room->random_point()->@*;
            if ( !$map->has_entity_at( $x, $y ) ) {
                my $e = Entities::healing_potion();
                $map->spawn( $e, $x, $y );
            }
        }
    ...
    my sub tunnel_between ( $map, $start, $end ) {
        ...
        my ( $x2, $y2 ) = @$end;
        if ( rand() < 0.5 ) {
            $map->_tiles_to_floor( [ min( $x1, $x2 ) .. max( $x1, $x2 ) ],
                [$y1] );
            $map->_tiles_to_floor( [$x2],
                [ min( $y1, $y2 ) .. max( $y1, $y2 ) ] );
        }
        else {
            $map->_tiles_to_floor( [ min( $x1, $x2 ) .. max( $x1, $x2 ) ],
                [$y2] );
            $map->_tiles_to_floor( [$x1],
                [ min( $y1, $y2 ) .. max( $y1, $y2 ) ] );
        }
    }
